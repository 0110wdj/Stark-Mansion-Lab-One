...
function extractResultBWCalc(result) {
  ...
  for (const img of images) {
    ...
    if (fileName.startsWith('orig_')) {
      ...
      entry.origClassProbability = Number.parseFloat(parts[3]);
    } else if (fileName.startsWith('adv_')) {
      entry.advFileName = fileName;
      entry.advFetchUrl = img.fetchUrl;
      entry.afterClass = parts[1];
      entry.afterClassProbability = Number.parseFloat(parts[3]);
    }
  }

  // 将 Map 转换为数组，使用文件名中的 id
  const imageList = Array.from(imageMap.entries())
    .map(([id, data]) => ({
      id: Number.parseInt(id, 10),
      ...data
    }))
    // 过滤掉没有配对成功的项
    .filter(item => item.origFileName && item.advFileName)
    // 按 id 排序
    .sort((a, b) => a.id - b.id);
