...
function serialize(root: TreeNode | null): string {
  ...
  if (root === null) { return '[]' }
  let i = 1;
  const buildArr = (root: TreeNode | null, arr: (number | null)[], i: number) => {
    if (root === null) {
      arr[i - 1] = null
      return
    }
    ...
    //   return
    // }
    buildArr(root.left, arr, 2 * i)
    buildArr(root.right, arr, 2 * i + 1)
  }
  buildArr(root, arr, i)
  // 超长数组序列化问题
  try {
    ...
    try {
      let resMsg = ''
      if (arr.length > 100) {
        for (let i = 0; i < arr.length; i = i + 500) {
          resMsg += JSON.stringify(arr.slice(i, i + 500)) + ','
        }
      }
      return '[' + resMsg.replaceAll('[', '').replaceAll(']', '') + ']'
    } catch (error) {
      let resMsg = '['
      let len = arr.length - 1
      for (let i = 0; i < len; i++) {
        const v = arr[i];
        const tar = (Number.isInteger(v)) ? (v) : 'null'
        resMsg += tar + ','
      }
      resMsg += (Number.isInteger(arr[len])) ? (arr[len]) : 'null'
      return resMsg
    }
  }
};
...
function deserialize(data: string): TreeNode | null {
  try {
    ...
    const buildTree = (root: TreeNode | null, i: number, arr: number[]): TreeNode | null => {
      if (root === null) {
        root = { val: arr[i - 1], left: null, right: null }
      }
      if ((2 * i > arr.length) || arr[2 * i - 1] === undefined || arr[2 * i - 1] === null) {
        root.left = null
      } else {
        root.left = buildTree(root.left, 2 * i, arr)
      }
      if (((2 * i + 1) > arr.length) || arr[2 * i] === undefined || arr[2 * i] === null) {
        root.right = null
      } else {
        root.right = buildTree(root.right, 2 * i + 1, arr)
      }
      return root
    }
    return buildTree(root, 1, arr)
  } catch (error) {
    return null
  }
};
